Raycast = {
    ...
    function Raycast:Cast(map_table, pos, alpha, screen_width)
        ...
        for i = 0,screen_width do
            ...
            if Raycast:dist(rays_h[i],pos) == -1 and Raycast:dist(rays_v[i],pos) == -1 then
                rays[i] = {
                    x = -1,
                    y = -1,
                    z = -1
                }
            elseif Raycast:dist(rays_h[i],pos) < Raycast:dist(rays_v[i],pos) then
                if Raycast:dist(rays_h[i],pos) == -1 then
                    rays[i] = rays_v[i]
                else
                    rays[i] = rays_h[i]
                end
            else
                if Raycast:dist(rays_v[i],pos) == -1 then
                    rays[i] = rays_h[i]
                else
                    rays[i] = rays_v[i]
                end
